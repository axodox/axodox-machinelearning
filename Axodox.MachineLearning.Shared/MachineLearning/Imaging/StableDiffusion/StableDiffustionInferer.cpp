...
{
  StableDiffusionInferer::StableDiffusionInferer(const Sessions::OnnxSessionParameters& parameters) :
  ...
  Tensor StableDiffusionInferer::RunInference(const StableDiffusionOptions& options, Threading::async_operation_source* async)
  {
    ...
    IoBinding binding{ *session };
    binding.BindOutput("out_sample", environment->MemoryInfo());
